...
total <- length(dd$Risk_Flag)
true <- length(dd$Risk_Flag[dd$Risk_Flag==1])
false <- length(dd$Risk_Flag[dd$Risk_Flag==0])
...
true_ind <- sample(seq_len(nrow(dd[dd$Risk_Flag==1])), size = true_smp_size)
false_ind <- sample(seq_len(nrow(dd[dd$Risk_Flag==0])), size = false_smp_size)
# Split the data frames
true_smp <- (dd[dd$Risk_Flag==1])[true_ind, ]
false_smp <- (dd[dd$Risk_Flag==0])[false_ind, ]
...
ExpData(data=dd,type=1)

# Structure of the data - Type = 2
ExpData(data=dd,type=2, fun = c("mean", "median", "var"))
```
...
ExpCTable(dd,Target="Risk_Flag",margin=1,clim=10,nlim=3,round=2,bin=NULL,per=F)
```
```{r}
#Distributions of Numerical variables
plot1 <- ExpNumViz(dd,target="Risk_Flag",type=1,nlim=5,fname=NULL,col=c("darkgreen","springgreen3","springgreen1"),Page=c(2,2),sample=5)
plot1[[1]]
```

```{r}
#Distributions of categorical variables
plot2 <- ExpCatViz(dd,target="Risk_Flag",fname=NULL,clim=15,col=c("slateblue4","slateblue1"),margin=2,Page = c(1,1),sample=5)
plot2[[1]]
```
...
dd$CITY <- as.data.frame(gsub("[[:punct:]]", "", as.matrix(dd$CITY)))
dd$STATE <- as.data.frame(gsub("[[:punct:]]", "", as.matrix(dd$STATE))) 
